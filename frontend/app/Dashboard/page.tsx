...
const Dashboard = () => {
  ...
  const filteredEventData = eventData.filter((event: EventItem) => {
    if (selectedOptions.includes("delivered") && event.event_type_id === 2) {
      return true;
    }
    if (selectedOptions.includes("not delivered") && event.event_type_id === 1) {
      return true;
    }
    return false;
  });
  
  const tableData = filteredEventData
    .filter(
      (event: EventItem) =>
        event.address_id !== null && event.address_id !== undefined
    )
    .map((event: EventItem) => ({
  ...
  const handleOptionToggle = (option: string) => {
    // Ensure option is explicitly typed as string
